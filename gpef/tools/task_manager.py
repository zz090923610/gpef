...
class TaskManager:
    def __init__(self, id):
        ...
        self.tmp_path = os.path.join(self.CURRENT_DIR, 'task_manager%d.pickle' % id)
        self.tasks_remain = []
        self.tasks_finished = []
        self.tasks_failed = []
    def print_status(self):
        print("TeskManager: %d remain, %d finished, %d failed." % (len(self.tasks_remain), len(self.tasks_finished), len(self.tasks_failed)))
    def generate_task_list(self, csv_path):
        if os.path.exists(self.tmp_path):
            try:
                self.load_tmp()
            except Exception as e:
                print("TeskManager.generate_task_list().self.load_tmp().Exception: %s" % e)
        else:
